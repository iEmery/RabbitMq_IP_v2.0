...
using RabbitMq_IP_v2.0_ClassLibrary.ServiceReference1;
...
namespace RabbitMq_IP_v2._0_ClassLibrary
{
    //  This is the core class that integrate the 3 systems by rabbitmq messaging solution.
    //  Every integration flow is identified by a UUID.
    class RabbitMq_IP
    {
        //NEW CUSTOMER
          ...
          string BASE_URL, string ACCOUNT, string PASSWORD, string SENDER)
        {
            return new Action(() => {
                ...
                while (true)
                {
                    ...
                    RabbitMq_IP_v2.0_ClassLibrary.Message_new_customer[] messages = new RabbitMq_IP_v2.0_ClassLibrary.Message_new_customer[check_list.Count()];
                    if (check_list.Count() != 0 || actual_list.Count() == 0)
                    {
                        ...
                        foreach (Bukimedia.PrestaSharp.Entities.customer c in check_list)
                        {
                            ...
                            RabbitMq_IP_v2.0_ClassLibrary.Message_new_customer message = new RabbitMq_IP_v2.0_ClassLibrary.Message_new_customer()
                            {
